...
class MyWorkOnProjectController extends Controller
{
    ...
    function markAsAccept(Request $request)
    {
        try {
            ...
            return redirect()->route('profile')->with(['message' => __("message.project_delivered_success"), 'type' => 'alert-success']);
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            return redirect()->back()->with(['message' => __('message.time_limit_exceeded'), 'type' => 'alert-success']);
        } catch (\Throwable $th) {
            //     //throw $th;
    ...
    function markAsReject(Request $request)
    {
        try {
            ...
            return redirect()->route('profile')->with(['message' => __('message.project_rejected'), 'type' => 'alert-danger']);
            $project->status = 'reject_receive';
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            return redirect()->route('myProject')->with(['message' =>  __('message.process.time_limit_exceeded'), 'type' => 'alert-success']);
        } catch (\Throwable $th) {
            return back()->with(['message' =>  __('message.page_not_found'), 'type' => 'alert-danger']);
        }
    }
    ...
    function markAsContinue($project_id)
    {
        try {
            ...
            $currentProject->save();
            return back()->with(['message' => __('message.project_resumed'), 'type' => 'alert-success']);
        } catch (\Illuminate\Http\Client\ConnectionException $e) {
            return redirect()->back()->with(['message' => __('message.time_limit_exceeded'), 'type' => 'alert-success']);
        } catch (\Throwable $th) {
            return back()->with(['message' =>  __('message.page_not_found'), 'type' => 'alert-danger']);
