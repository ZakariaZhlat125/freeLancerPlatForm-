                    ...
                    <div class="main flex-1 flex flex-col overflow-auto ">
                        ...
                        <div class="flex-1 flex flex-row-reverse h-full overflow-auto">
                                        ...
                                        @foreach ($users as $user)
                                            ...
                                            <a href="{{ route('inbox.show', $user->id) }}">
                                                <div class="entry cursor-pointer transform hover:scale-105 duration-300 transition-transform bg-white mb-4 rounded p-4 flex shadow-md"
                                                    wire:click="getUser({{ $user->id }})" id="user_{{ $user->id }}">
                                                    <div class="flex-2">
                                                        <div class="w-12 h-12 relative">
                                                            ...
                                                            @if ($user->is_online)
                                                                <span
                                                                    class="absolute w-4 h-4 bg-primary-green rounded-full right-0 bottom-0 border-2 border-white"></span>
                                                            @endif
                                                        </div>
                                                    </div>
                            ...
                            <div class="chat-area flex-1 flex flex-col overflow-auto">
                                ...
                                <div class="messages flex-1 flex flex-col flex-col-reverse overflow-auto scrollbar"
                                    id="style-7" dir="ltr">
                                    @if (filled($messages))
                                        @foreach ($messages as $message)
                                            <div
                                                class="message mb-4 flex @if ($message->user_id !== auth()->id()) received @else sent @endif">
                                                ...
                                                @else
                                                    <div class="flex-1 px-2">
                                                        <div
                                                            class="inline-block bg-primary-light-gray rounded-full p-2 px-6 text-gray-700">
                                                            <span>{{ $message->message }}</span>
                                                        </div>
                                                        {{-- @if (isPhoto($message->file))
                                                            <div class="w-100 my-2">
                                        ...
                                        {{-- @if ($file)
                                            <div class="mb-2">
                                                ...
                                                <button type="button" wire:click="resetFile"
                                                    class="btn bg-primary-light-pink text-right btn-sm">
                                                    حذف {{ $file->getClientOriginalName() }}<i class="fa fa-times"></i>
                                                </button>
                                            </div>
                                        @endif --}}
                                            ...
                                            <div class="flex-2 w-32 p-2 flex content-center items-center">
                                                @if (empty($file))
                                                    <div class="flex-1 text-center">
                                                        <label
                                                            class="w-4 flex flex-col items-center px-4 py-6 rounded-md tracking-wide cursor-pointer ease-linear transition-all">
                                                            ...
                                                            <input type="file" class="hidden" type="file"
                                                                wire:model="file" />
                                                        </label>
                                                    </div>
                                                @endif
